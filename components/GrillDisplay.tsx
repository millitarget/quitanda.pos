...
import { ChefHat, ListOrdered, ListChecks, RefreshCw } from 'lucide-react';
import type { Order } from '../App';
import { filterOrdersForStation, aggregateItems } from '../utils/stations';
import { formatCustomizationsForDisplay, groupOrderItems } from '../utils/orderGrouping';

interface GrillDisplayProps {
  orders: Order[];
  loading?: boolean;
  onRefresh?: () => void;
}

export function GrillDisplay({ orders, loading = false, onRefresh }: GrillDisplayProps) {
  const [view, setView] = useState<'byOrder' | 'aggregated'>('byOrder');

  const grillOrders = useMemo(() => filterOrdersForStation(orders, 'grill'), [orders]);
  const aggregated = useMemo(() => {
    const allItems = grillOrders.flatMap(o => o.items);
    ...
    <div className="h-full flex flex-col bg-background">
      <div className="flex items-center justify-between px-4 py-3 border-b bg-card sticky top-0 z-50 mobile-safe-area">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold">Churrasqueira</h2>
        ...
        <div className="flex items-center gap-2">
          <Tabs value={view} onValueChange={(v) => setView(v as any)}>
            <TabsList>
              <TabsTrigger value="byOrder" className="gap-1"><ListOrdered className="h-3 w-3"/>By order</TabsTrigger>
              <TabsTrigger value="aggregated" className="gap-1"><ListChecks className="h-3 w-3"/>Totals</TabsTrigger>
            </TabsList>
          </Tabs>
          {onRefresh && (
            <Button
        ...
        {view === 'byOrder' ? (
                ...
                <Card key={order.id} className="rounded-lg shadow-xs border border-border/60">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-full bg-yellow-600 text-white flex items-center justify-center">
          ...
          <div className="p-3 md:p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            {aggregated.map((line, idx) => (
              <Card key={idx} className="rounded-lg shadow-xs border border-border/60">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <Badge variant="default" className="text-xs px-2 py-0 rounded">{line.quantity}x</Badge>
                      <span className="font-medium">{line.name}</span>
                    </div>
                    <Badge variant="secondary" className="text-xs">€{line.totalPrice.toFixed(2)}</Badge>
                  </div>
                  {line.customizations.length > 0 && (
                    <div className="ml-1 mt-1 space-y-1">
                      {formatCustomizationsForDisplay(line.customizations).map((c, cIdx) => (
                        <div key={cIdx} className="text-xs text-muted-foreground">{c}</div>
                      ))}
                    </div>
                  )}
                </CardContent>
