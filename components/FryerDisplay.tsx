...
export function FryerDisplay({ orders, loading = false, onRefresh }: FryerDisplayProps) {
  const fryerOrders = useMemo(() => filterOrdersForStation(orders, 'fryer'), [orders]);
    ...
    <div className="h-full flex flex-col bg-background">
      <div className="flex items-center justify-between px-4 py-3 border-b bg-card sticky top-0 z-50 mobile-safe-area">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold">Fritadeira</h2>
          <Badge variant="secondary" className="text-xs py-0.5 px-2 rounded-md">
            {fryerOrders.length} orders
          </Badge>
        </div>
        {onRefresh && (
          <Button
